...
with open("BricksExtras_Persian_Translations.py", "r", encoding="utf-8") as f:
    content = f.read()

# Extract translations dictionary using regex
match = re.search(r'translations = \{(.*?)\n\s*\}', content, re.DOTALL)
if match:
    dict_content = match.group(1)
    # Count quotes to validate
    translations_dict = eval("{" + dict_content + "}")
else:
...
print(f"Done! Translated: {translated} strings, Total: {total} strings")
